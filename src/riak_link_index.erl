...
-export([postcommit/1, precommit/1]).
...
precommit(Object) ->
    ...
    case is_updated(Object) of
      true ->
	  OldLinksToMe =
	      get_index_links(riak_object:get_metadatas(Object)),
	  ...
	  case StorageMod:get(Bucket, Key) of
	    {ok, OldRO} ->
		OldLinksToMe =
		    get_index_links(riak_object:get_metadatas(OldRO));
	    _ -> OldLinksToMe = []
	  end,
    ...
    LinksToRemove = ordsets:subtract(OldLinksToMe,
				     NewLinksToMe),
    LinksToAdd = ordsets:subtract(NewLinksToMe,
				  OldLinksToMe),
...
add_links(StorageMod, Links, Bucket, Key) ->
    lists:foreach(fun ({{IndexB, IndexK},
			<<(?IDX_PREFIX), Tag/binary>>}) ->
			  add_link(StorageMod, IndexB, IndexK,
				   {{Bucket, Key}, Tag})
		  end,
		  Links).
...
add_link(StorageMod, Bucket, Key, Link) ->
    update(fun (Object) ->
		   ...
		   riak_object:update_metadata(IO1,
					       dict:store(?MD_LINKS,
							  [Link] ++ Links,
...
remove_links(StorageMod, Links, Bucket, Key) ->
    lists:foreach(fun ({{IndexB, IndexK},
			<<(?IDX_PREFIX), Tag/binary>>}) ->
			  remove_link(StorageMod, IndexB, IndexK,
				      {{Bucket, Key}, Tag})
		  end,
		  Links).
...
remove_link(StorageMod, Bucket, Key, Link) ->
    update(fun (Object) ->
		   ...
		   IO1 = riak_object:update_value(Object, <<>>),
		   riak_object:update_metadata(IO1,
					       dict:store(?MD_LINKS, UpdLinks,
							  riak_object:get_update_metadata(IO1)))
	   end,
	   StorageMod, Bucket, Key).

update(Fun, StorageMod, Bucket, Key) ->
    Updated = case StorageMod:get(Bucket, Key) of
		{ok, Object} -> Fun(Object);
		_ -> Fun(riak_object:new(Bucket, Key, <<>>))
	      end,
...
index_contents(Bucket, Contents) ->
    ...
    [index_contents_1(V, IndexHooks) || V <- Contents].

index_contents_1({MD, Value}, IndexHooks) ->
    case dict:find(?MD_DELETED, MD) of
      {ok, "true"} -> {remove_idx_links(MD), Value};
      _ ->
...
remove_idx_links(MD) ->
    ...
    case dict:find(?MD_LINKS, MD) of
      ...
      {ok, Links} ->
	  dict:store(?MD_LINKS,
		     [V || V <- Links, remove_idx_links_1(V)], MD)
    end.

remove_idx_links_1({_, <<(?IDX_PREFIX), _/binary>>}) ->
    false;
remove_idx_links_1(_) -> true.

compute_indexed_md(MD, Value, IndexHooks) ->
    lists:foldl(fun ({struct, PropList} = IndexHook,
		     MDAcc) ->
			{<<"tag">>, Tag} = proplists:lookup(<<"tag">>,
							    PropList),
			...
			KeepLinks = [V
				     || V <- Links,
					compute_indexed_md_1(V, IdxTag)],
			NewLinksSansTag = try apply_index_hook(IndexHook, MD,
							       Value)
					  of
					    {erlang, _, {ok, IL}}
						when is_list(IL) ->
						...
						IL;
					    _Val ->
						error_logger:error_msg("indexing function returned ~p",
								       [_Val]),
						[]
					  catch
					    _:_ ->
						error_logger:error_msg("exception invoking indexing function",
								       []),
						[]
					  end,
			ResultLinks = [compute_indexed_md_2(V1, IdxTag)
				       || V1 <- NewLinksSansTag]
					++ KeepLinks,
...
compute_indexed_md_1({{_, _}, TagValue}, IdxTag) ->
    TagValue =/= IdxTag.

compute_indexed_md_2({Bucket, Key}, IdxTag)
...
get_index_hooks(Bucket) ->
    ...
    IndexHooks = proplists:get_value(link_index,
				     BucketProps, []),
    case IndexHooks of
      <<"none">> -> {ok, []};
      {struct, Hook} -> {ok, [{struct, Hook}]};
      IndexHooks when is_list(IndexHooks) -> {ok, IndexHooks};
      V ->
	  error_logger:error_msg("bad value in bucket_prop ~p:link_index: ~p",
				 [Bucket, V]),
	  {ok, []}
    end.
...
invoke_hook(Mod0, Fun0, undefined, MD, Value)
    ...
    try {erlang, {Mod, Fun}, Mod:Fun(MD, Value)} catch
      Class:Exception ->
	  {erlang, {Mod, Fun},
	   {'EXIT', Mod, Fun, Class, Exception}}
    end;
invoke_hook(undefined, undefined, JSName, MD, Value)
    when JSName /= undefined ->
    {js, JSName,
     riak_kv_js_manager:blocking_dispatch(?JSPOOL_HOOK,
					  {{jsfun, JSName},
					   [jsonify_metadata(MD), Value]},
					  5)};
...
jsonify_metadata(MD) ->
    MDJS = fun ({LastMod, Now = {_, _, _}}) ->
		   ...
		   {LastMod,
		    list_to_binary(httpd_util:rfc1123_date(calendar:now_to_local_time(Now)))};
	       ({<<"Links">>, Links}) ->
...
jsonify_metadata_list(List) ->
    Classifier = fun ({Key, _}, Type)
			 when is_binary(Key) orelse is_list(Key), Type /= array,
			      Type /= string ->
			 struct;
		     (C, Type)
			 when is_integer(C), C >= 0, C =< 256, Type /= array,
			      Type /= struct ->
			 string;
		     (_, _) -> array
		 end,
    case lists:foldl(Classifier, undefined, List) of
      struct ->
	  {struct,
	   [{if is_list(Key) -> list_to_binary(Key);
		true -> Key
	     end,
